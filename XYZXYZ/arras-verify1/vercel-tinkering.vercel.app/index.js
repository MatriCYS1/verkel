...
function handleVerifiedPacket(...data) {
  ...
  for (let i = 0; i < data.length / 5; i ++) {
    document.querySelector("#verifyName" + i).addEventListener("click", async () => {
      ...
      elements.blockUser.dataset.name = data[i * 5];
      elements.blockUser.disabled = data[i * 5 + 1] !== "user";
      elements.blockUser.innerText = blockedUsers.includes(data[i * 5]) ? "Unblock User" : "Block User";
    });
  }
}
  ...
  message = message.replace(
    ...
    (gmatch, emoteId) => {
      ...
      const width = (
        ...
        emoteId === "insane" || emoteId === "puzzles"
      ) ? 90 : 30;
...
function displayChatMessage(data) {
  ...
  try {
    ...
    .children].find((child) => child.tagName.toLowerCase() === "b")?.addEventListener?.("click", async () => {
      ...
      elements.blockUser.disabled = data.rank !== "user";
...
async function updateSandboxPlayers() {
  const response = await fetch("https://qrp6ujau11f36bnm-c.uvwx.xyz:8443/2222/status");
